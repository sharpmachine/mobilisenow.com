...
jQuery(document).ready(function($) {
    function reset_form(ev) {
        ...
        var answer = confirm('Are you sure you want to reset the theme options?');
    ...
    $('#theme-opts select.social-select').live('change', function(ev) {
...
this_val.substring(1);
    ...
    $('#theme-opts a.social-add-cta').live('click', function(ev) {
        ev.preventDefault();
    ...
    $('#theme-opts a.social-delete-cta').live('click', function(ev) {
        ev.preventDefault();
    ...
    // rss stuff
    $('#theme-opts input.social-rss-default').live('click', function(ev) {
        $(this).parent().siblings('input.social-rss-url').attr('disabled', true).val('');
    });
    
    $('#theme-opts input.social-rss-custom').live('click', function(ev) {
        $(this).parent().siblings('input.social-rss-url').attr('disabled', false);
    });
    
    // antisocial link
    $('#theme-opts .antisocial-cta').live('click', function(ev) {
        ev.preventDefault();
