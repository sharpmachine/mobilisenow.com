// WCT javascript Functions

function statusUpdates(action,$){
	switch(action){
		case "stop":
		...
		case "cont":
			xhr = $.ajax({
					  ...
					  success		:	function(output){
						  if(output.action == "cont"){
							  $("#searchProg").html(output.progData);
							  statusUpdates(output.action, $);
						  }
						  else if(output.action == "stop"){
							  $("#searchProg").html("<pre>"+output.error+"</pre>");
							  statusUpdates(output.action, $);
						  }
					  }
				  });
